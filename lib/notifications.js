...
const POST_EXPIRING_SOON_MESSAGE = "Your post will expire soon";
...
export async function notifyPostExpiringSoon(email) {
  const normalized = normalizeNotificationEmail(email);
  if (!db || !normalized) return;
  try {
    await addDoc(collection(db, COLLECTION), {
      message: POST_EXPIRING_SOON_MESSAGE,
      email: normalized,
      createdAt: new Date(),
    });
  } catch (err) {
    devError("notifyPostExpiringSoon", err);
  }
}
...
export function notifyPostExpiringSoonOncePerSession(itemId, email) {
  if (typeof window === "undefined" || !itemId || !email) return;
  const key = `exp-notify-item-${itemId}`;
  if (sessionStorage.getItem(key)) return;
  sessionStorage.setItem(key, "1");
  void notifyPostExpiringSoon(email);
}
