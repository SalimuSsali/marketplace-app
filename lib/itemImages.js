...
/**
 * Direct browser → R2 PUT needs TLS + CORS to `*.r2.cloudflarestorage.com`.
 * Some devices (older Windows/Chrome, strict networks) fail with
 * ERR_SSL_VERSION_OR_CIPHER_MISMATCH. Default: upload via our server only
 * (`POST /api/r2-upload`). Set `NEXT_PUBLIC_R2_BROWSER_DIRECT_UPLOAD=1` to
 * opt into the fast presigned-PUT path when your users’ browsers can reach R2.
 */
function useBrowserDirectR2Upload() {
  if (typeof process === "undefined" || !process.env?.NEXT_PUBLIC_R2_BROWSER_DIRECT_UPLOAD) {
    return false;
  }
  return String(process.env.NEXT_PUBLIC_R2_BROWSER_DIRECT_UPLOAD).trim() === "1";
}
  ...
  try {
    ...
    try {
      ...
      if (typeof console !== "undefined") {
        console.warn(
          "[upload] Direct R2 PUT failed (likely missing bucket CORS policy or network timeout). Falling back to server upload. Error:",
          putErr,
        );
      }
    }
