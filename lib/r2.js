...
// Prefer IPv4 for DNS in this process (Vercel→R2 often fails on IPv6 / TLS alert 40).
// Custom `https.Agent` lookup caused "Invalid IP address: undefined" with Node’s caller contract.
...
export function r2UploadErrorUserMessage(err) {
  ...
  if (/SSL alert number 40|handshake failure|EPROTO/i.test(rawMsg)) {
    return "Could not complete TLS to Cloudflare R2 (often IPv6 routing). This app sets DNS to prefer IPv4 — redeploy and try again. Confirm R2_ACCOUNT_ID and API token on Vercel.";
  }
