...
/** Shared agent: many hosts (e.g. Vercel) get a bad IPv6 path to R2 → TLS alert 40. IPv4 works. */
let r2Ipv4Agent;

function getR2Ipv4HttpsAgent() {
  if (r2Ipv4Agent) return r2Ipv4Agent;
  r2Ipv4Agent = new https.Agent({
    keepAlive: true,
    maxSockets: 32,
    /** Prefer IPv4 — Vercel→R2 over IPv6 often ends in TLS alert 40. */
    lookup(hostname, _opts, cb) {
      if (typeof hostname !== "string" || !hostname) {
        cb(new TypeError("Invalid hostname for DNS"));
        return;
      }
...
export function getR2Client(config) {
  return new S3Client({
    ...
    requestHandler: new NodeHttpHandler({
      httpsAgent: getR2Ipv4HttpsAgent(),
...
export function r2UploadErrorUserMessage(err) {
  ...
  if (/SSL alert number 40|handshake failure|EPROTO/i.test(rawMsg)) {
    return "Could not complete TLS to Cloudflare R2 (common if the server used a bad IPv6 route). This build forces IPv4 — redeploy and try again. Also confirm R2_ACCOUNT_ID and API token on Vercel.";
  }
...
/**
 * Presigned one-shot PUT: plain TLS + unsigned payload. Preferred when it works.
 * @param {string} putUrl
 * @param {Buffer} buf
 * @param {string} contentType
 */
function uploadViaPresignedHttpsPut(putUrl, buf, contentType) {
