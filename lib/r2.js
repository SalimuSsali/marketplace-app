...
export function r2UploadErrorUserMessage(err) {
  ...
  if (/SSL alert number 40|handshake failure|EPROTO/i.test(rawMsg)) {
    return "Could not complete TLS to Cloudflare R2. In Vercel → Project → Environment variables, set R2_ACCOUNT_ID (R2 overview), R2_ACCESS_KEY_ID + R2_SECRET_ACCESS_KEY (S3 API token for this bucket), R2_BUCKET_NAME, and R2_PUBLIC_BASE_URL (your r2.dev / custom public URL, not the *.r2.cloudflarestorage.com API host). Redeploy after any change.";
  }
...
/**
 * Presigned one-shot PUT to R2. Uses the URL hostname and normal Node TLS (SNI = hostname).
 * Connecting to a resolved IPv4 + manual Host was causing TLS / SSL alert 40 on some serverless
 * hosts; hostname-based `https.request` matches browsers and is reliable on Vercel.
 * @param {string} putUrl
 * @param {Buffer} buf
...
async function uploadViaPresignedHttpsPut(putUrl, buf, contentType) {
  ...
  if (!u.hostname) throw new TypeError("Presigned URL has no host");
...
/**
 * Server-side upload: try presigned HTTPS PUT (plain Node `https` to hostname — best TLS on
 * Vercel), then fall back to SDK PutObject if needed.
 * @param {Buffer} body
 * @param {string} contentType
 */
export async function uploadImageToR2(config, key, body, contentType) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `R2 upload failed. Could not presign: ${errorMessageWithCause(signErr)}. SDK PutObject: ${errorMessageWithCause(sdkErr)}`,
      );
    }
  }
