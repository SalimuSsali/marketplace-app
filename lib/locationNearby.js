...
export function filterAndSortItemsByNearby(items, needle) {
  ...
  const filtered = items.filter((item) =>
    itemLocationMatchesNeedle(getItemLocationSearchText(item), n),
  );
  return filtered.sort(
    (a, b) =>
      locationMatchScore(getItemLocationSearchText(b), n) -
      locationMatchScore(getItemLocationSearchText(a), n),
  );
}
