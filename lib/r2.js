...
/**
 * Server-side object upload. Do NOT use S3Client.send(PutObject) here: newer
 * @aws-sdk/client-s3 builds can emit aws-chunked / flexible-checksum request
 * bodies that R2’s edge rejects mid-handshake (Node reports it as EPROTO / SSL
 * alert 40). A normal single-part HTTP PUT to a presigned URL matches the
 * browser path and is reliable.
 * @param {Buffer} body
...
export async function uploadImageToR2(config, key, body, contentType) {
  ...
  // Use node:https instead of global fetch. Undici often turns TLS/DNS failures
  // into an unhelpful `TypeError: fetch failed` without surfacing .cause.
    ...
    const req = https.request(
      {
        ...
        minVersion: "TLSv1.2",
        // Avoid broken IPv6 routes from some serverless hosts (symptoms: EPROTO / fetch failed).
